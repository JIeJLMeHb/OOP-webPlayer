class PlayerApp {
    ...
    prev(){this.index=(this.index-1+this.tracks.length)%this.tracks.length;this.load(this.index);this.audio.play();}
    next(){this.index=(this.index+1)%this.tracks.length;this.load(this.index);this.audio.play();}
    updateProgress(){
    ...
    formatTime(sec){if(!isFinite(sec))return'0:00';const m=Math.floor(sec/60),s=Math.floor(sec%60);return`${m}:${s.toString().padStart(2,'0')}`;}
}
